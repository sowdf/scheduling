...
request.get('http://project.4399ued.com/apiDoc/ios/_book/',(err,httpResponse,body)=>{
    if(err){
...
function getApi(link){
    request.get('http://project.4399ued.com/apiDoc/ios/_book/' + link,(err,httpResponse,body)=>{
        if(err){
            return console.log(err);
        }
        const $ = cheerio.load(body);
        ...
        request.post({url:'http://d.4399ued.com/api/content/add', form:
            {
                "categoryId":'59e4274d91f89967cfcbf768',
                apiName:apiName,
