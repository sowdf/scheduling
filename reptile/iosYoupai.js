...
request.get('http://project.4399ued.com/apiDoc/youpai/ios/_book/',(err,httpResponse,body)=>{
    if(err){
...
function getApi(link){
    request.get('http://project.4399ued.com/apiDoc/youpai/ios/_book/' + link,(err,httpResponse,body)=>{
        if(err){
            return console.log(err);
        }
        const $ = cheerio.load(body);
        ...
        request.post({url:'http://d.4399ued.com/api/content/add', form:
            {
                "categoryId":'59e4285891f89967cfcbf7b0',
                apiName:apiName,
